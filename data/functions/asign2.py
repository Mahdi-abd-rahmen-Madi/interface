...
def find_nearest_address_in_parcel(roof_idx, roofs_gdf, address_gdf, parcel_gdf, roof_to_parcel_mapping, address_sindex):
    ...
    If no parcel is found, use a spatial index to narrow down candidates.
    """
    ...
    if parcel_id and parcel_id in parcel_gdf.index:
        ...
        candidate_addresses = addresses_in_parcel if not addresses_in_parcel.empty else None
    
    if candidate_addresses is None:
        # Fallback: Use roof's bounding box to narrow down candidates
        ...
        if candidate_addresses.empty:
            candidate_addresses = address_gdf  # Last resort: all addresses
...
def process_chunk(roof_indices, roofs_gdf, address_gdf, parcel_gdf, roof_to_parcel_mapping, address_sindex):
    """
...
if __name__ == "__main__":
    ...
    address_sindex = address_gdf.sindex
