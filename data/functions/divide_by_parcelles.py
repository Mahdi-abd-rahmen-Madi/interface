...
#warnings.filterwarnings("ignore", message="driver FlatGeobuf does not support open option DRIVER")
...
def drop_unnecessary_attributes(divided_roofs_gdf, parcelles_gdf):
    ...
    for col in divided_roofs_gdf.columns:
        if col.lower() in ["ofid"]:  # Replace with actual columns to drop
...
def save_chunk_to_file(chunk, temp_dir, index):
    ...
    if "FlatGeobuf" in fiona.supported_drivers:
        temp_path = f"{temp_dir}/chunk_{index}.fgb"
        driver = "FlatGeobuf"
    elif "GPKG" in fiona.supported_drivers:
    ...
    try:
        # Save without passing unsupported parameters
...
def process_intersection_from_file(temp_path, parcelles_gdf):
    ...
    """
    print(f"Processing chunk: {temp_path}")  # Add debug statement
    try:
        divided_roofs_chunk = gpd.read_file(temp_path) #, driver=driver)
        result = gpd.overlay(
            divided_roofs_chunk,
            parcelles_gdf,
            how='intersection',
...
def divide_roofs_by_parcelles(divided_roofs_path, parcelles_path, output_path, num_processes=None):
    ...
    try:
        ...
        chunk_size = max(len(divided_roofs) // (num_processes * 8), 1)  # Reduce chunk size
        ...
        # Filter to retain only Polygon or MultiPolygon geometries
        polygons = result[result.geometry.type.isin(['Polygon', 'MultiPolygon'])]

        # Handle non-Polygon geometries separately
        non_polygons = result[~result.geometry.type.isin(['Polygon', 'MultiPolygon'])]
        if not non_polygons.empty:
            non_polygons_output_path = output_path.replace(".shp", "_non_polygons.shp")
            non_polygons.to_file(non_polygons_output_path, driver="ESRI Shapefile")
            print(f"Non-polygon geometries saved to {non_polygons_output_path}.")

        # Validate geometries
        invalid_count = sum(not geom.is_valid for geom in polygons.geometry)
        if invalid_count > 0:
            print(f"Found {invalid_count} invalid geometries. Attempting to repair...")
            polygons['geometry'] = polygons.geometry.apply(lambda geom: geom.buffer(0) if not geom.is_valid else geom)

        # Save the filtered polygons to the main output Shapefile
        polygons.to_file(output_path, driver="ESRI Shapefile")
        print(f"Final polygon result saved to {output_path}.")
        save_end = time.time()
...
if __name__ == "__main__":
    ...
    output_path = "/home/mahdi/interface/data/output/divide/roofs_divided_by_parcelles6.shp"
