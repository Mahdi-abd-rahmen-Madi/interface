...
# Step 3: Align Target Polygons with Gradual Buffer Increase
def align_target_to_reference_with_gradual_buffer(target_gdf, reference_polygons, initial_buffer=10, max_buffer=100, step=10):
    """
    Align target polygons to reference polygons using a gradually increasing buffer distance.
    Args:
        ...
        initial_buffer (float): Initial buffer distance in meters.
        max_buffer (float): Maximum buffer distance in meters.
        step (float): Incremental step size for buffer distance.
    Returns:
        GeoDataFrame: Aligned GeoDataFrame with preserved attributes and alignment status.
    """
    logging.info("Aligning target polygons with gradually increasing buffer distances...")
    ...
    for idx, row in target_gdf.iterrows():
        ...
        # Try aligning with increasing buffer distances
        buffer_distance = initial_buffer
        is_aligned = False
        while buffer_distance <= max_buffer:
            buffered_references = [poly.buffer(buffer_distance) for poly in reference_polygons]

            for ref, buffered_ref in zip(reference_polygons, buffered_references):
                if target_geom.within(buffered_ref):  # Check against the buffered reference
                    attributes["alignment_status"] = "aligned"
                    attributes["overlap_area"] = target_geom.area
                    attributes["reference_id"] = str(id(ref))[:10]  # Convert ID to string and truncate to 10 chars
        ...
        if not is_aligned:
            # No match found within the maximum buffer distance, mark as "not_aligned"
            attributes["alignment_status"] = "not_aligned"
            attributes["overlap_area"] = 0
            attributes["reference_id"] = None
            attributes["buffer_distance"] = None
            unaligned_data.append({**attributes, "geometry": target_geom})

    logging.info(f"Matched {len(aligned_data)} target polygons with gradually increasing buffer distances.")
...
def save_aligned_results(data, output_path, output_format="shp", crs="EPSG:2154"):
    ...
    if data:
        ...
        # Rename columns to fit the 10-character limit for ESRI Shapefile
        gdf.rename(columns={
            "alignment_status": "status",
            "overlap_area": "ovlp_area",
            "reference_id": "ref_id",
...
def process_shapefiles(target_shapefile_path, reference_shapefile_path, output_dir, output_format="shp", target_crs="EPSG:2154", initial_buffer=10, max_buffer=50, step=10):
    """
    Process two shapefiles to align target polygons with reference polygons using a gradual buffer increase.
    Args:
        target_shapefile_path (str): Path to the target shapefile.
        reference_shapefile_path (str): Path to the reference shapefile.
    ...
    # Align target polygons with gradual buffer increase
    aligned_data = align_target_to_reference_with_gradual_buffer(target_gdf, reference_polygons, initial_buffer=initial_buffer, max_buffer=max_buffer, step=step)
...
def batch_process_shapefiles(output_dir, output_format="shp", target_crs="EPSG:2154", initial_buffer=10, max_buffer=50, step=10):
    """
    Process hardcoded shapefiles for alignment with a gradual buffer increase.
    """
    ...
    process_shapefiles(target_shapefile_path, reference_shapefile_path, output_dir, output_format=output_format, target_crs=target_crs, initial_buffer=initial_buffer, max_buffer=max_buffer, step=step)
...
if __name__ == "__main__":
    ...
    batch_process_shapefiles(output_dir, output_format="shp", target_crs="EPSG:2154", initial_buffer=10, max_buffer=50, step=10)