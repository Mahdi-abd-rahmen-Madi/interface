...
# Step 3: Align Target Polygons to Buffered Reference Polygons (Allow Multiple Alignments)
def align_target_to_reference_with_buffer(target_gdf, reference_polygons, buffer_distance=10):
    """
    Align target polygons to reference polygons using a buffer distance, allowing multiple alignments per reference.
    Args:
        ...
        buffer_distance (float): Distance in meters to expand the reference polygons.
    Returns:
        GeoDataFrame: Aligned GeoDataFrame with preserved attributes and alignment status.
    """
    logging.info("Aligning target polygons to buffered reference polygons (allowing multiple alignments)...")

    # Create buffered versions of the reference polygons
    buffered_references = [poly.buffer(buffer_distance) for poly in reference_polygons]

    # Initialize lists to store aligned and unaligned results
    aligned_data = []
    unaligned_data = []
    ...
    for idx, row in target_gdf.iterrows():
        ...
        # Check if the target polygon is within any buffered reference polygon
        is_aligned = False
        for ref, buffered_ref in zip(reference_polygons, buffered_references):
            if target_geom.within(buffered_ref):  # Check against the buffered reference
                ...
                attributes["overlap_area"] = target_geom.area  # Full area since it's within the buffer
                attributes["reference_id"] = id(ref)  # Add reference ID for tracking
    ...
    logging.info(f"Matched {len(aligned_data)} target polygons within {buffer_distance} meters of reference polygons.")
...
def process_shapefiles(target_shapefile_path, reference_shapefile_path, output_dir, output_format="shp", target_crs="EPSG:2154", buffer_distance=10):
    """
    Process two shapefiles to align target polygons with reference polygons using a buffer, allowing multiple alignments.
    Args:
        target_shapefile_path (str): Path to the target shapefile.
        reference_shapefile_path (str): Path to the reference shapefile.
    ...
    # Align target polygons to reference polygons with buffer
    aligned_data = align_target_to_reference_with_buffer(target_gdf, reference_polygons, buffer_distance=buffer_distance)
...
def batch_process_shapefiles(output_dir, output_format="shp", target_crs="EPSG:2154", buffer_distance=10):
    """
    Process hardcoded shapefiles for alignment with a buffer, allowing multiple alignments.
    """
    ...
    process_shapefiles(target_shapefile_path, reference_shapefile_path, output_dir, output_format=output_format, target_crs=target_crs, buffer_distance=buffer_distance)
...
if __name__ == "__main__":
    ...
    batch_process_shapefiles(output_dir, output_format="shp", target_crs="EPSG:2154", buffer_distance=10)