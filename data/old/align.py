...
def align_target_to_reference(target_polygons, reference_polygons):
    """
    Align target polygons to reference polygons.
    Args:
        target_polygons (list): List of Shapely Polygons from the target shapefile.
        reference_polygons (list): List of Shapely Polygons from the reference shapefile.
    Returns:
        list: List of tuples (target_polygon, matched_reference_polygon).
    """
    matches = []
    ...
    for target in target_polygons:
        if target.intersects(reference_union):
            # Find the most overlapping reference polygon
            best_match = None
            max_overlap_area = 0
            for ref in reference_polygons:
                if target.intersects(ref):
                    overlap_area = target.intersection(ref).area
            ...
            if best_match:
                ...
                matches.append((target, simplified_ref))
...
def save_aligned_results(matches, output_path, output_format="shp", crs="EPSG:2154"):
    ...
    Args:
        matches (list): List of tuples (target_polygon, matched_reference_polygon).
    ...
    for target, reference in matches:
        data.append({
            "geometry": target,  # Only store the target geometry
...
def process_shapefiles(target_shapefile_path, reference_shapefile_path, output_dir, output_format="shp", target_crs="EPSG:2154"):
    ...
    target_polygons = [Polygon(polygon) for polygon in target_gdf.geometry]
    reference_polygons = [Polygon(polygon) for polygon in reference_gdf.geometry]
    # Align target polygons to reference polygons
    matches = align_target_to_reference(target_polygons, reference_polygons)
