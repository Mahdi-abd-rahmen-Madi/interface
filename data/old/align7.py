...
# Step 4: Align Target Polygons to Reference Polygons with 75% Overlap Condition
def align_target_to_reference(original_target_polygons, buffered_target_polygons, reference_polygons, min_overlap_ratio=0.75):
    """
    Align original target polygons to reference polygons using buffered polygons for overlap checks.
    Only include matches where the overlap area is at least `min_overlap_ratio` of the target polygon's area.
    Args:
        ...
        min_overlap_ratio (float): Minimum overlap ratio (default: 0.75).
    Returns:
    ...
    for original, buffered in zip(original_target_polygons, buffered_target_polygons):
        if buffered.intersects(reference_union):
            ...
            if best_match:
                # Check if the overlap area meets the minimum ratio requirement
                overlap_ratio = max_overlap_area / original.area
                if overlap_ratio >= min_overlap_ratio:
                    simplified_ref = best_match.simplify(tolerance=0.5, preserve_topology=True)
                    matches.append((original, simplified_ref))
                    logging.debug(f"Matched target polygon to reference polygon with {overlap_ratio:.2%} overlap.")

    logging.info(f"Matched {len(matches)} target polygons to reference polygons with at least {min_overlap_ratio:.0%} overlap.")
    return matches
...
def save_aligned_results(matches, output_path, output_format="shp", crs="EPSG:2154"):
    ...
    for target, reference in matches:
        data.append({
            ...
            "ovlp_ratio": target.intersection(reference).area / target.area
        })
    if data:
        gdf = gpd.GeoDataFrame(data, geometry="geometry", crs=crs)
        gdf.rename(columns={"ovlp_area": "overlap_area", "ovlp_ratio": "overlap_ratio"}, inplace=True)  # Rename for clarity
...
def process_shapefiles(target_shapefile_path, reference_shapefile_path, output_dir, buffer_distance=1.0, min_overlap_ratio=0.75, output_format="shp", target_crs="EPSG:2154"):
    """
    ...
    # Align original target polygons to reference polygons using buffered polygons for overlap checks
    matches = align_target_to_reference(original_target_polygons, buffered_target_polygons, reference_polygons, min_overlap_ratio=min_overlap_ratio)
...
def batch_process_shapefiles(output_dir, buffer_distance=1.0, min_overlap_ratio=0.75, output_format="shp", target_crs="EPSG:2154"):
    """
    ...
    process_shapefiles(target_shapefile_path, reference_shapefile_path, output_dir, buffer_distance=buffer_distance, min_overlap_ratio=min_overlap_ratio, output_format=output_format, target_crs=target_crs)
...
if __name__ == "__main__":
    ...
    batch_process_shapefiles(output_dir, buffer_distance=1.0, min_overlap_ratio=0.75, output_format="shp", target_crs="EPSG:2154")